...

{
    ...
    public class TooltipManager : MelonMod
    {
        ...
        {
            public static void Prefix(Il2Cpp.TooltipItemManager __instance, Il2Cpp.ItemDataUnpacked data, Il2Cpp.TooltipItemManager.SlotType type, UnityEngine.Vector2 tooltipOffset, UnityEngine.Vector3 position, UnityEngine.GameObject opener, UnityEngine.Vector2 openerSize)
            {
                if (data != null)
                {
                    //FallenUtils.Log("OpenTooltip : ");
                    originalLoreText = data.LoreText ?? string.Empty;
                    HandleTooltipUpdate(data);
                }
            }

            public static void Postfix(Il2Cpp.TooltipItemManager __instance, Il2Cpp.ItemDataUnpacked data, Il2Cpp.TooltipItemManager.SlotType type, UnityEngine.Vector2 tooltipOffset, UnityEngine.Vector3 position, UnityEngine.GameObject opener, UnityEngine.Vector2 openerSize)
            {
                if (data != null)
                {
